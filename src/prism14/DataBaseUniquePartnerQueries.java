...
public class DataBaseUniquePartnerQueries {

	Connection conn;
	CallableStatement selectAll;
	
	public DataBaseUniquePartnerQueries(boolean isPreviousYear) {
		try {
		...
		selectAll = (isPreviousYear) ? conn.prepareCall("{CALL get_partner_customer_previous(?, ?, ?, ?, ?)}") : 
			conn.prepareCall("{CALL get_partner_customer(?, ?, ?, ?, ?)}");
		}
	...
	getAll(String node, String name, String finYear, 
		...
		try {
			...
			results = new ArrayList<DataBaseUniquePartner>();
				while (resultSet.next()) {
					results.add(new DataBaseUniquePartner 
