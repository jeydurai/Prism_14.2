...
public class DataBaseUniqueCustomerQueries {

	Connection conn;
	CallableStatement selectAll;
	
	public DataBaseUniqueCustomerQueries(boolean isPreviousYear) {
		try {
		...
		selectAll = (isPreviousYear) ? conn.prepareCall("{CALL get_customer_previous(?, ?, ?, ?, ?)}") :
			conn.prepareCall("{CALL get_customer(?, ?, ?, ?, ?)}");
		}
	...
	getAll(String node, String name, String finYear, 
		...
		try {
			...
			results = new ArrayList<DataBaseUniqueCustomer>();
				while (resultSet.next()) {
					results.add(new DataBaseUniqueCustomer 
